...
import { HookService, RESOLVER_HOOK, GenericGapiResolversType, ON_REQUEST_HANDLER, GRAPHQL_PLUGIN_CONFIG, Boom } from '@gapi/core';
...
@Module({
    services: [
        AuthService,
        {
            ...
            deps: [HookService],
            useFactory: (hookService: HookService) => (resolver: GenericGapiResolversType) => {
                ...
                async (next, context, request: Request, h: ResponseToolkit, err: Error) => {
                    if (request.headers.authorization && request.headers.authorization !== 'undefined') {
                        try {
                            ...
                            config.graphqlOptions.context['user'] = config.graphqlOptions.context;
                        }
                        catch (e) {
